...
@Injectable()
export class RoleStatusService extends SqlService {
	constructor(
	...
	async many({ user, ...payload }): Promise<any> {
		try {
			const cachedData = await this.cacheService.get([ 'role', 'status', 'many', payload ]);
			...
			await this.cacheService.set([ 'role', 'status', 'many', payload ], output);
	...
	async one({ user, ...payload }): Promise<any> {
		try {
			const cachedData = await this.cacheService.get([ 'role', 'status', 'one', payload ]);
			...
			if (output) {
				await this.cacheService.set([ 'role', 'status', 'one', payload ], output);
			}
			if (!output) {
	...
	async drop({ user, ...payload }): Promise<any> {
		try {
			...
			await this.cacheService.clear([ 'role', 'status', 'one', payload ]);
	...
	async update({ user, ...payload }): Promise<any> {
		...
		try {
			...
			await this.updateWithId(this.roleStatusRepository, payload);
